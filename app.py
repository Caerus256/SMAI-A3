...
/* hide the model-select buttons' native styling */
.model-btn > button {
    background: var(--card) !important;
...
def render_sidebar(df):
    ...
    # Leaderboard = model selector (single component)
    st.sidebar.markdown(
        '<p class="section-label">Model Leaderboard</p>', unsafe_allow_html=True
    )
    ...
    # Initialise selection in session state
    if "selected_model" not in st.session_state:
        st.session_state["selected_model"] = available[0]["type"]

    # Render each model as a button row
    for m in MODEL_REGISTRY:
        ...
        # Clickable button right below to select (only for available models)
        if avail:
